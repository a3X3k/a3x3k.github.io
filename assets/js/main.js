...
function renderResearchGraph(){
  ...
  const top=[...filtered].sort((a,b)=>(b.citations||0)-(a.citations||0)).slice(0,isMobile?36:50);
  const groups=[...new Set(top.map(p=>researchGroupFor(p,mode)))];
  const W=isMobile?720:1000,H=isMobile?860:620,cx=W/2,cy=H/2;
  svg.setAttribute('viewBox', `0 0 ${W} ${H}`);
  svg.setAttribute('preserveAspectRatio','xMidYMid meet');
  const groupRadiusX=isMobile?230:320;
  const groupRadiusY=isMobile?300:220;
  const paperBase=isMobile?54:78;
  const paperExtra=isMobile?58:85;
  const maxPerGroup=isMobile?6:9;
  ...
  groups.forEach((g,gi)=>{
    const ang=2*Math.PI*gi/groups.length-Math.PI/2,gx=cx+Math.cos(ang)*groupRadiusX,gy=cy+Math.sin(ang)*groupRadiusY,col=palette[gi%palette.length],del=gi*55;
    ...
    const papers=top.filter(p=>researchGroupFor(p,mode)===g).slice(0,maxPerGroup);
    papers.forEach((p,i,arr)=>{
      const sp=Math.min(isMobile?.48:.34,.16+(arr.length*.022)),pa=ang+(i-(arr.length-1)/2)*sp,pr=paperBase+Math.min(paperExtra,(p.citations||0)*(isMobile?1.9:2.8)),px=Math.max(30,Math.min(W-30,gx+Math.cos(pa)*pr)),py=Math.max(36,Math.min(H-36,gy+Math.sin(pa)*pr)),pr2=(isMobile?8:7)+Math.min(isMobile?15:19,Math.sqrt((p.citations||0)+1)*(isMobile?1.7:2.1)),pd=del+120+i*38;
...
function initAurora(){/* performance-safe: aurora animation is handled by CSS only */}
...
/* === PERFORMANCE-SAFE MICRO ENHANCEMENTS === */
function initPerformanceSafeTheme(){
  if(document.documentElement.dataset.fastThemeReady==='1')return;
  document.documentElement.dataset.fastThemeReady='1';
  const reduce=matchMedia('(prefers-reduced-motion: reduce)').matches;
  if(reduce)return;
  const progress=document.createElement('div');
  progress.className='scroll-cinema';
...
document.addEventListener('DOMContentLoaded',initPerformanceSafeTheme);
